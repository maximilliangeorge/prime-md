...
describe("resolveAllPremises", () => {
  it("returns remote nodes for resolved premises", async () => {
    ...
    const nodes = [
      parseNode(path.join(remoteFixtures, "root-with-remote.md"), remoteFixtures),
    ];

    const remoteNodes = await resolveAllPremises(nodes, remoteFixtures, emptyManifest);
  ...
  it("recursively resolves local premises within a remote repo", async () => {
    ...
    const remoteNodes = await resolveAllPremises([node], remoteFixtures, emptyManifest);
